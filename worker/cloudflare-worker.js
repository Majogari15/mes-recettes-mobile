...
// - limitation de débit par adresse IP via la liaison Rate Limiting de
//   Cloudflare (nécessite une configuration manuelle, voir README.md —
//   fonctionne sans limitation tant que cette étape n'est pas faite,
//   plutôt que de casser le Worker)
...
export default {
  async fetch(request, env) {
    ...
    // script qui n'envoie simplement pas cet en-tête (d'où la
    // limitation de débit ci-dessous, une protection complémentaire).
    const requestOrigin = request.headers.get("Origin");
    if (requestOrigin && requestOrigin !== ALLOWED_ORIGIN) {
      return new Response("Origin not allowed", { status: 403, headers: corsHeaders });
    }

    // Limitation de débit par adresse IP — voir worker/README.md pour
    // la configuration exacte via Wrangler (liaison "RATE_LIMITER") ;
    // cette liaison n'est actuellement pas configurable depuis le
    // tableau de bord Cloudflare. Si la liaison n'est pas configurée
    // (`env.RATE_LIMITER` absent), le Worker continue de
    // fonctionner normalement sans limitation plutôt que de casser
    // toutes les requêtes — pour ne jamais empêcher l'application de
    // fonctionner tant que cette étape de configuration n'a pas été
    // faite manuellement.
    if (env && env.RATE_LIMITER) {
      ...
      try {
        ...
        if (!success) {
          return new Response("Too many requests, please try again in a minute.", { status: 429, headers: corsHeaders });
        }
      } catch (e) {
        // Ne bloque jamais une requête légitime pour une erreur du
        // service de limitation lui-même — mieux vaut laisser passer
        // que de casser l'application pour tous ses utilisateurs.
      }
    }
